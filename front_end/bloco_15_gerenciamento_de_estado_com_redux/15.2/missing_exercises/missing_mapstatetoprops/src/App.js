  ...
  first() {
    ...
    return (
      <div className="flip-card-inner">
        ...
        >
          <div className="">
            <img src={ patrick } alt="Patrick" height="320px" />
          </div>
          <div className="">
            <img src={ gary } alt="Gary" height="120px" />
          </div>
        </div>
        <ul className="flip-card-back list-group">
          <h2>Ingredientes</h2>
          {receiveIngredients.map((element, index) => (
    ...
    return (
      <div className="flip-card-inner">
        ...
        <ul className="flip-card-back list-group">
          ...
          {receiveOrder.map((element, index) => (
            <li key={ index }>{element}</li>
          ))}
        </ul>
      </div>
  ...
  render() {
    return (
      <div className="row spongeBob">
        <div className="flip-card left">{this.first()}</div>
        <div className="flip-card middle">{this.second()}</div>
        <div className="flip-card right">{this.third()}</div>
      </div>
    );
  }
}
