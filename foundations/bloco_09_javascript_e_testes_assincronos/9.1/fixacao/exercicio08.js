...
const findCharmander = ({name}) => name === 'Charmander';
const findSquirtle = ({name}) => name === 'Squirtle';

const action = (error, message) => {
  if (message === null) return console.log(error);

  console.log(message);
}  
getPokemonDetails(findBulbasaur, action);
